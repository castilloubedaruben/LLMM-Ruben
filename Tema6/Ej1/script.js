...
function mostrarPokemon(datos) {
    let name_stats=datos.stats.map(stats => stats.stat.name)
    let base_stats=datos.stats.map(stats => stats.base_stat)

    let estadistica =""
    for (let i = 0; i < name_stats.length; i++) {
        estadistica+=name_stats[i] + " - " + base_stats[i] + "<br>"
    }
    ...
    contenedor.innerHTML = `
        ...
        <p><strong>Stats:</strong> ${estadistica}</p>
    `;
}
